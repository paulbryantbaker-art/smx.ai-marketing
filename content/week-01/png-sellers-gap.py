#!/usr/bin/env python3
"""
smbx.ai LinkedIn Single Image — Monday Post 1B: Sellers Companion
"$16.4M Gap" — single 1080×1350 image, the standalone Field Note version
of the HVAC comparison story.

Built to the smbx.ai DESIGN_LANGUAGE.md (2026-04-15 redesign) with
Fibonacci spacing throughout. One cohesive published page:
  Masthead → HookEyebrow → Sora hook → 2-column $ compare → gap row
  → Sora punchline → portrait byline.

Usage:
    python3 png-sellers-gap.py            # light (default)
    python3 png-sellers-gap.py dark
"""
...
/* ── Gap row (single line beneath the two columns) ──────────────────── */
.gap-row {
...
<div class="canvas"><div class="content">
  ...
  <div class="compare-cols">
    <div class="compare-side">
      <div class="label">Seller A</div>
      <div class="big num">$12.4M</div>
      <div class="mult num">3.1&times; on $4M EBITDA</div>
    </div>
    <div class="compare-side">
      <div class="label accent">Seller B</div>
      <div class="big accent num">$28.8M</div>
      <div class="mult num">7.2&times; on $4M EBITDA</div>
    </div>
  </div>
  ...
  <div class="punchline">
    The PE firms that passed on one <span class="accent">fought</span> over the other.
  </div>
...
print(f"Rendering MON-1B ({MODE}) at {W}x{H} @ {DPR}x...")
with sync_playwright() as p:
    ...
    out = OUT_DIR / f"MON-1B-sellers-gap-single-{MODE}.png"
